...
describe("deposit akv", () => {
  const request = `
  mutation DepositAKV($input: DepositAKVInput!) {
    depositAKV(input: $input)
  }
  `;
  ...
  test("success", async () => {
    ...
    expect(ret.data.depositAKV).toBeTruthy();
  });
  test("illegal state", async () => {
    useCase.throwErrorForDeposit = new IllegalStateUseCaseError("test");
    const ret = await send();
    expectGraphqlError(ret as ExecutionResult, "ILLEGAL_STATE");
  });
  test("invalid argument", async () => {
    useCase.throwErrorForDeposit = new InvalidArgumentUseCaseError("test");
    const ret = await send();
    expectGraphqlError(ret as ExecutionResult, "INVALID_ARGUMENT");
  });
  test("unhandled", async () => {
    useCase.throwErrorForDeposit = new Error("test");
    const ret = await send();
