...
describe("stopRecruitment", () => {
  const request = `
  mutation StopRecruitArcadeMachine($input: StopRecruitArcadeMachineInput!) {
    stopRecruitArcadeMachine(input: $input) {
      id
    }
  }
  `;
  ...
  test("success", async () => {
    mockUseCase.returnValueForStop = {
      id: "1",
      state: "IN_AKIVERSE",
      createdAt: new Date(),
      updatedAt: new Date(),
    ...
    const stopRecruitArcadeMachine = ret.data.stopRecruitArcadeMachine;
    expect(stopRecruitArcadeMachine.id).toBe("1");
  });
  test("not found", async () => {
    mockUseCase.throwErrorForStop = new NotFoundUseCaseError(
      "test",
      "GameCenter",
    );
    const ret = await send();
  ...
  test("illegal state", async () => {
    mockUseCase.throwErrorForStop = new IllegalStateUseCaseError("test");
    const ret = await send();
    expectGraphqlError(ret as ExecutionResult, "ILLEGAL_STATE");
  });
  test("permission denied", async () => {
    mockUseCase.throwErrorForStop = new PermissionDeniedUseCaseError();
    const ret = await send();
    expectGraphqlError(ret as ExecutionResult, "PERMISSION_DENIED");
  });
  test("unknown error", async () => {
    mockUseCase.throwErrorForStop = new Error("test");
    const ret = await send();
